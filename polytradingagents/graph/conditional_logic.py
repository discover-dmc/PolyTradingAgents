# PolyTradingAgents/graph/conditional_logic.py

from polytradingagents.agents.utils.agent_states import AgentState
...
class ConditionalLogic:
    ...
    def build_liquidity_router(self, analyst_node_names: list[str]):
        """Return a routing function that fans out to analysts or short-circuits to SKIP.

        Returns a closure over the analyst node names so the router can fan out
        to all analysts in parallel (LangGraph supports list returns from
        conditional edge functions as of 0.2+).

        Args:
            analyst_node_names: List of analyst node name strings to fan out to.
        """
        portfolio_manager = NodeNames.PORTFOLIO_MANAGER

        def _router(state: AgentState):
            if state.get("liquidity_ok", False):
                # Return list of node names → LangGraph fans out in parallel
                return analyst_node_names
